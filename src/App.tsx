...
import { getHoldingKey, usePrevious, useStatePaire, useStateWithLS } from './hookUtils'
import { scrollToNext, useHoverWords } from './reader'
...
const APP = () => {
    ...
    const [scrollTop, currentLine, blockL, blockR, onScrollHandle, stopScroll, overscan, setUpdata] = useScroll(_overscan, widthCount, heightCount)

    const [onKeyDown, onKeyUp, clickType] = useKey(overscan.get.bot, widthCount, currentLine, heightCount)
        ...
        <>
            ...
            <div
                ...
                {...{
                    ...
                    onMouseOver: e => SET_hoverWord(getWord(e.target as Element)!),
                    // onMouseOver: e => {
...
` +
                ...
                {(() => {
                    ...
                    const vals = Object.entries(styles).reduce((all, [key, val]) => (all += `${key}: ${val};\n`), '\n')
    ...
    function SETWRAP_selectArr(arr: item[]) {
        SET_selectArr(arr.sort((l, r) => (r.count != l.count ? r.count - l.count : r.i - l.i)))
    }
}
function APPwrap() {
    console.time()
